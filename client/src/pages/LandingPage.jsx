...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden relative selection:bg-primary/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 flex flex-col md:flex-row items-center justify-between relative z-10">
        <motion.div 
          initial="hidden"
          animate="visible"
          variants={{
        ...
        >
          <motion.div variants={defaultAnimations} className="inline-block px-4 py-1.5 rounded-full bg-white/60 backdrop-blur-sm border border-orange-200 text-orange-600 font-bold text-sm shadow-sm font-sans">
            🚀 The #1 Campus Food App
          </motion.div>
          <motion.h1 variants={defaultAnimations} className="text-6xl md:text-7xl font-heading font-extrabold text-textPrimary leading-[1.1] tracking-tight">
            Craving something <br/> 
            <span className="text-gradient">delicious?</span>
          </motion.h1>
          <motion.p variants={defaultAnimations} className="text-xl text-textSecondary leading-relaxed max-w-xl">
            Skip the lines. Order ahead, track your delivery in real-time, and enjoy your favorite campus meals hot and fresh.
          </motion.p>
          <motion.div variants={defaultAnimations} className="flex flex-wrap gap-4 pt-4">
            <Link to="/login" className="bg-primary hover:bg-orange-600 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-xl shadow-orange-500/20 hover:shadow-orange-500/40 hover:-translate-y-1 flex items-center gap-2">
        ...
        <motion.div 
          ...
          className="md:w-[45%] mt-16 md:mt-0 flex justify-center lg:justify-end relative"
        >
          {/* Main Hero Visual Card */}
          <div className="w-[320px] h-[400px] md:w-[400px] md:h-[500px] glass rounded-3xl p-6 flex flex-col items-center justify-center relative z-20 animate-float shadow-2xl overflow-hidden">
             
             <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-white/10 z-0 rounded-3xl"></div>
             
             {/* Decorative food images or 3D elements inside card */}
             <div className="z-10 bg-white/60 w-36 h-36 flex items-center justify-center rounded-full shadow-inner mb-6 backdrop-blur-sm">
                <span className="text-[80px] drop-shadow-2xl leading-none block">🍔</span>
             </div>
             
             <div className="z-10 text-center w-full">
               <div className="h-4 w-3/4 bg-gray-200/80 rounded-full mx-auto mb-3"></div>
               <div className="h-3 w-1/2 bg-gray-200/80 rounded-full mx-auto"></div>
             </div>
             
             {/* Floating badge */}
             <div className="absolute right-4 bottom-12 md:right-8 md:bottom-16 bg-white shadow-xl shadow-green-500/10 rounded-2xl p-4 flex items-center gap-3 z-30 animate-pulse">
